...
# Lista de colunas onde as classes estão localizadas
...
# Quantidade de base de dados testadas
qty_databases=4

# Quantidade de vezes que executará cada cenário de teste
qty_test=10

# Quantidade de cenários de números de processos
# qty_proc_test=12
qty_proc_test=2
...
for i in $(seq 0 $((qty_databases-1))); do
...
for i in $(seq 0 $((qty_databases-1))); do
    ...
    for j in $(seq 0 $((qty_proc_test-1))); do
        echo "Executando testes da base ${path_databases[$i]} para ${num_procs[$j]} processos ..."
        for k in $(seq 0 $((qty_test-1))); do
            echo "mpirun -np ${num_procs[$j]} bin/./kmeans_mpi_app ${path_databases[$i]}.bin ${num_features[$i]} ${class_count[$i]} >> ${out_files[$i]}"
            mpirun -np ${num_procs[$j]} bin/./kmeans_mpi_app ${path_databases[$i]}.bin ${num_features[$i]} ${class_count[$i]} >> ${out_files[$i]}
